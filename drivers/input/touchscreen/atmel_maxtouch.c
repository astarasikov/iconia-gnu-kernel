...
int process_message(u8 *message, u8 object, struct mxt_data *mxt)
{
	...
	static u8 reported_touches[MXT_MAX_NUM_TOUCHES];
	u8 touch_number;
	...
	case MXT_TOUCH_MULTITOUCHSCREEN_T9:
		touch_number = message[MXT_MSG_REPORTID] -
			mxt->rid_map[report_id].first_rid;
		/* If this touch number is already collected, send the events */
		if(reported_touches[touch_number]) {
			memset(reported_touches, 0, MXT_MAX_NUM_TOUCHES);
			process_T9_message(NULL, mxt, 1);
		}
		reported_touches[touch_number] = 1;
